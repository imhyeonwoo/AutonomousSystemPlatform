...
//   • 주기 20 Hz
//   - [MODIFIED] Implemented velocity planning in the mission state for faster waypoint traversal.
...
public:
    ...
    prev_err_e_(0.0), prev_err_n_(0.0)
  {
    ...
    declare_parameter("landing_h_thresh",     0.35);
    declare_parameter("touchdown_alt",        0.15);
    declare_parameter("kp_horizontal",        1.2);
    declare_parameter("kd_horizontal",        0.45);
    declare_parameter("descent_speed_min",    0.6);
    declare_parameter("descent_speed_max",    1.2);
    // ★ [NEW] Parameters for velocity planning
    declare_parameter("target_cruise_speed",  8.0); // m/s
    declare_parameter("deceleration_distance", 10.0); // m
    ...
    // ★ [NEW] Get velocity planning parameters
    target_cruise_speed_ = get_parameter("target_cruise_speed").as_double();
    deceleration_distance_ = get_parameter("deceleration_distance").as_double();
...
private:
  ...
  /* ───── 공통 Publish ───── */
  // ★ [MODIFIED] Now enables velocity control flag
  void publishCtrlMode()
  {
    px4_msgs::msg::OffboardControlMode m{};
    m.timestamp = usec();
    m.position = true; // For altitude control
    m.velocity = true; // For horizontal velocity control
    off_ctrl_pub_->publish(m);
  }

  // ★ [MODIFIED] This is now a generic setpoint publisher
  void publishSetpoint(const px4_msgs::msg::TrajectorySetpoint& sp)
  {
      publishCtrlMode();
      traj_pub_->publish(sp);
  }

  // This function is now used for states other than MISSION
  ...
  void hover(float x, float y, float z) { publishPositionSetpoint(x, y, z); }
  ...
  // ★ [MODIFIED] Mission logic now uses velocity control
  void mission()
  {
    if (wp_idx_ >= wps_.size()) {
      hold_x_ = last_pos_->x; hold_y_ = last_pos_->y; hold_z_ = last_pos_->z;
      state_ = State::LANDING_APPROACH;
      RCLCPP_INFO(get_logger(), "All waypoints completed. Switching to LANDING_APPROACH.");
      return;
    }
    ...
    float dE = tgt.e - enu->e;
    float dN = tgt.n - enu->n;
    float dU = tgt.u - enu->u;
    float dist_to_target = std::hypot(dE, dN);

    // Waypoint acceptance criteria
    if (dist_to_target < 2.5f) { // Wider acceptance radius for smooth transition
      if (wp_idx_ == wps_.size() - 1 && !lookdown_sent_) {
        std_msgs::msg::String s; s.data = "LOOK_DOWN";
        gimbal_mode_pub_->publish(s);
        lookdown_sent_ = true;
      }
      ++wp_idx_;
      RCLCPP_INFO(get_logger(), "Waypoint reached, moving to WP %zu", wp_idx_);
      return;
    }

    // --- Velocity Planning Logic ---
    float current_target_speed;
    if (dist_to_target < deceleration_distance_) {
      // Deceleration zone: scale speed by distance
      current_target_speed = std::max(1.5f, (float)(target_cruise_speed_ * (dist_to_target / deceleration_distance_)));
    } else {
      // Cruise/acceleration zone
      current_target_speed = target_cruise_speed_;
    }

    // Calculate velocity vector in ENU
    float vel_e = (dE / dist_to_target) * current_target_speed;
    float vel_n = (dN / dist_to_target) * current_target_speed;

    // --- Create TrajectorySetpoint message ---
    px4_msgs::msg::TrajectorySetpoint sp{};
    sp.timestamp = usec();
    
    // Set horizontal positions to NAN to enable velocity control
    sp.position[0] = NAN;
    sp.position[1] = NAN;
    // Keep altitude control with position setpoint
    sp.position[2] = last_pos_->z - dU;

    // Set velocity fields (ENU to NED conversion)
    sp.velocity[0] = vel_n; // NED North (m/s)
    sp.velocity[1] = vel_e; // NED East (m/s)
    sp.velocity[2] = NAN;
  ...
  void landing_approach()
  {
    ...
    // Use position control for landing approach
    publishPositionSetpoint(last_pos_->x + err_n,
                            last_pos_->y + err_e,
  ...
  void landing_descent()
  {
    ...
    double dt = (this->now() - prev_t_).seconds();
    if (dt <= 0.0) dt = 0.05;
    prev_t_ = this->now();
    ...
    // Use position control for landing descent
    publishPositionSetpoint(last_pos_->x + ctrl_n,
                            last_pos_->y + ctrl_e,
                            last_pos_->z - err_u);
  }
  ...
  // ★ [NEW] Member variables for velocity planning
  double target_cruise_speed_, deceleration_distance_;
