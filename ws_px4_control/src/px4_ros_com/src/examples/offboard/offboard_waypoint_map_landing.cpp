...
//   • Waypoint 비행 + 마커 기반 정밀 착륙(Approach → Descent → Touchdown)
//   • 마지막 WP 도달 시 짐벌 LOOK_DOWN → 카메라 수직 고정
//   • /desired_body_yaw_enu → ENU→NED 변환 후 TrajectorySetpoint.yaw 주입
//   • 주기 20 Hz
// --------------------------------------------------------------------------
...
public:
    ...
    prev_err_e_(0.0), prev_err_n_(0.0)
  {
    ...
    declare_parameter("takeoff_alt",          3.0);
    ...
    declare_parameter("landing_h_thresh",     0.20);

    // [유지] 착지 고도는 안전하게 유지합니다.
    declare_parameter("touchdown_alt",        0.25);

    // [핵심 수정] P 게인을 낮춰 오버슛과 진동을 줄입니다. (가장 중요한 변경)
    declare_parameter("kp_horizontal",        1.4);

    // [핵심 수정] D 게인을 살짝 높여 제동(Damping) 역할을 강화하고 움직임을 안정시킵니다.
    declare_parameter("kd_horizontal",        0.45);

    // [핵심 수정] 최종 착지 속도를 낮춰 매우 부드럽게 만듭니다.
    declare_parameter("descent_speed_min",    0.4);

    // [핵심 수정] 최대 하강 속도를 대폭 낮춰 제어기가 안정적으로 반응할 시간을 확보합니다.
    declare_parameter("descent_speed_max",    2.0);
    ...
    timer_ = create_wall_timer(50ms,
              std::bind(&OffboardWaypointMapLanding::onTimer, this));
  }
...
private:
  ...
  std::optional<ENU> enuNow()
  {
    try{
      auto tf = tf_buf_.lookupTransform(
                  map_frame_, base_frame_, tf2::TimePointZero, 20ms);
  ...
  void sendCmd(uint16_t cmd,float p1=0.f,float p2=0.f)
  {
    ...
    cmd_pub_->publish(c);
    RCLCPP_INFO(get_logger(),"CMD 0x%04X sent (p1=%.1f)",cmd,p1);
  }
  ...
  void onTimer()
  {
    ...
    if(state_!=prev_state_){
      static const char* names[]={
        "WAIT_SIGNAL","INIT","TAKEOFF","MISSION",
        "LANDING_APPROACH","LANDING_DESCENT","TOUCHDOWN","LAND"};
      RCLCPP_INFO(get_logger(),"▶ STATE  %s → %s",
                  names[int(prev_state_)], names[int(state_)]);
      prev_state_=state_;
    }
  ...
  void init()
  {
    if(off_cnt_<5){ hover(last_pos_->x,last_pos_->y,last_pos_->z); ++off_cnt_; return; }
  ...
  void mission()
  {
    if(wp_idx_>=wps_.size()){
      ...
      state_=State::LANDING_APPROACH; return;
    }
    ...
    publishSet(last_pos_->x+dN, last_pos_->y+dE, last_pos_->z-dU);
  }
  ...
  void landing_approach()
  {
    ...
    if(h_err<landing_h_thresh_){
      ...
      if(std::fabs(err_u)<0.40){
        ...
        prev_t_=this->now();
        state_=State::LANDING_DESCENT; return;
      }
    }
    publishSet(last_pos_->x+err_n, last_pos_->y+err_e,
  ...
  void landing_descent()
  {
    ...
    double dt=(this->now()-prev_t_).seconds(); if(dt<=0.0) dt=0.05; prev_t_=this->now();
  ...
  void pubMarkers()
  {
    ...
    for(size_t i=0;i<wps_.size();++i){
      ...
      m.color.r=m.color.b=1.0; m.color.a=1.0;
  ...
  State state_;
  State prev_state_{State::WAIT_SIGNAL};
